...
function GenerateMap() {
	...
	var blocks = [13, 25, 32, 40, 47];

	for (var i = 0; i < blocks.length; i++) {
		map[blocks[i]] = BLOCK_OBSTACLE;
		map[MAP_SIZE * MAP_SIZE - 1 - blocks[i]] = BLOCK_OBSTACLE;
	}
	/*set trail*/
	var trails_1 = [0, 1, 3, 4, 5, 6, 7, 8, 12, 14, 15, 23, 24, 26, 27, 35, 36, 37, 38, 39, 41, 42, 43,
		48, 49, 50, 51, 52, 53, 54, 59, 60, 61, 62, 63, 64, 65, 70, 71, 72, 74, 75, 76
	];

	for (var i = 0; i < trails_1.length; i++) {
		map[trails_1[i]] = BLOCK_PLAYER_1_TRAIL;
	}

	var trails_2 = [33, 34, 44, 45, 46, 55, 56, 57, 58, 66, 67, 68, 69, 77, 78, 79, 81,
		82, 83, 84, 85, 86, 87, 89, 90, 91, 92, 93, 94, 96, 97, 98, 99, 100, 101, 102,
		109, 110, 111, 112, 113, 120
	];

	for (var i = 0; i < trails_2.length; i++) {
		map[trails_2[i]] = BLOCK_PLAYER_2_TRAIL;
	}


	// Player 1 start position
	playerPos[TURN_PLAYER_1] = new Position (9, 0);
	map[9] = BLOCK_PLAYER_1;

	// Player 2 start position
	playerPos[TURN_PLAYER_2] = new Position (4, MAP_SIZE - 1);
	map[114] = BLOCK_PLAYER_2;
}
